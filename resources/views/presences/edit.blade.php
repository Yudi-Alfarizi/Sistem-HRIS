                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item" aria-current="page"><a href="{{ route('presences.index') }}">Daftar
                                    Kehadiran</a></li>
                ...
                <div class="card-body">
                    <form action="{{ route('presences.update', $presence->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control datetime" name="check_in"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control datetime" name="check_out"
                                value="{{ old('check_out', $presence->check_out) }}" required>
                            @error('check_out')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control date" name="date"
                                value="{{ old('date', $presence->date) }}" required>
                            @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-select" required>
                                ...
                                <option value="hadir" {{ old('status', $presence->status) == 'hadir' ? 'selected' : '' }}>
                                    Hadir</option>
                                <option value="izin" {{ old('status', $presence->status) == 'izin' ? 'selected' : '' }}>
                                    Izin</option>
                                <option value="sakit" {{ old('status', $presence->status) == 'sakit' ? 'selected' : '' }}>
                                    Sakit</option>
                                <option value="alpa" {{ old('status', $presence->status) == 'alpa' ? 'selected' : '' }}>
                                    Alpa</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <a href="{{ route('presences.index') }}" class="btn btn-secondary">Kembali ke Daftar Kehadiran</a>
                    </form>
                </div>
            </div>
        </section>
